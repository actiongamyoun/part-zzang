...
export default function CheckPage({ record, project, user, onBack, onSaved }) {
  ...
  async function toggleStep(step) {
    ...
    const { data } = await supabase
      ...
      .select()
    if (data) {
      setSteps(prev => prev.map(s => s.id === step.id ? { ...s, ...data[0] } : s))
    }
  }
  ...
  async function handlePhotoUpload(e) {
    ...
    for (const file of files) {
      ...
      const path = `records/${record.id}/${Date.now()}.${ext}`
    ...
    <div className="page">
      <div className="topbar">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>호선 {project.hull_no}</div>
        </div>
      </div>
          ...
          {steps.map((step, i) => (
            <button key={step.id}
              ...
              <div style={{ flex: 1 }}>
                ...
                {step.completed_date && (
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{step.completed_date}</div>
                )}
              </div>
            </button>
          ))}
        ...
        <div className="card">
          ...
          <input type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)}
            style={{
              ...
              borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontSize: 15
            }}/>
        </div>
