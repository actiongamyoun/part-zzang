...
export default function InspectorDetailPage({ inspector, user, onBack, onSelectRecord }) {
  const [records, setRecords] = useState([])
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState('all')
  const [groupBy, setGroupBy] = useState('hull')

  useEffect(() => { load() }, [inspector.inspector_id])

  async function load() {
    setLoading(true)
    const { data } = await supabase
      .from('check_records')
      .select(`*, check_items(name, display_order), companies(name), hulls(hull_no, project_id, projects(name))`)
      .eq('inspector_id', inspector.inspector_id)
  ...
  if (groupBy === 'hull') {
    ...
    visibleRecords.forEach(r => {
      const key = r.companies?.name || '미배정'
      if (!groups[key]) groups[key] = []
      groups[key].push(r)
    })
  } else {
  ...
  return (
    <div className="page">
      <div className="topbar">
        ...
        <div style={{ flex: 1 }}>
          <div className="topbar-title">👤 {inspector.inspector_name}</div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{inspector.company_name || '소속 없음'}</div>
        </div>
      </div>
      ...
      <div className="content">
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4 }}>
          ...
          {[{ k: 'hull', l: '호선별' }, { k: 'company', l: '협력사별' }, { k: 'item', l: '항목별' }].map(g => (
                ...
                <button key={rec.id} className="card" onClick={() => onSelectRecord(rec)}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>
                      {groupBy === 'item' ? `호선 ${rec.hulls?.hull_no || '?'}` : rec.check_items?.name}
                    </div>
                    <span className={`badge ${st.cls}`}>{st.label}</span>
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                    {groupBy !== 'company' && <span>🏢 {rec.companies?.name || '미배정'}</span>}
                    {groupBy !== 'hull' && <span>🚢 호선 {rec.hulls?.hull_no || '?'}</span>}
